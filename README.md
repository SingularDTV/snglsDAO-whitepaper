...
Comparison between Bitcoin/Ethereum and Content Mining
Platform Manifesto
We ask, “what can our platform do for content, not what content can do for our platform.”
The choice is up to the people
WEB3 Tech Stack
SNGLS 2.0 Utility and Incentive Game
Five Types of Participants
Users (who buy and view content)
Content Creators (who add new entries to the system)
Validators
Developers
Third party service providers
SNGLS DAO
Adaptive Platform
Three Beginning Controls of DAO on Platform
Treasury Control
...
Mechanism 
Flow of System
Homepage Content Display (Curated Storefronts)
...
VII. Glossary
Breaker DApp: A high end content portal that sits on top of the SNGLS media distribution protocol. Handles the processes that need a legal entity for the traditional Hollywood world and banking system charging a 20% service fee.
SNGLS Media Distribution Protocol: A trustless global media distribution protocol anyone can come in and build on top of spurring a decentralized entertainment economy, controlled by the SNGLS DAO.
Tech Stack: A set of software that provides the infrastructure for a technology system.
Token Utility: A set of roles the token has in a specific tokenized ecosystem.
Incentive Game: A payoff provided as a reward for a person in some activity which does not directly provide them with benefit.  
Users: Content consumers who use the platform.
Validators: Entities that vote on content being added, blocked, or removed from the platform.
Content creator: Entities that create content for the platform, i.e. artists 
Developers: Programers that maintain the platform. 
Third party service providers: Any service that is built on top of the distribution portal.
Adaptive platform: A platform that adapts to the needs of the system organically through the DAO. 
Decentralized Autonomous Organization (DAO): Organization represented by rules encoded as a computer program that is transparent and controlled by stakeholders.
Treasury: Repository where the Transaction Fees and SNGLS that are from Staking for curation are stored and controlled by the DAO.
Validation Fee: Minimum amount paid to curators to vote yes or no on a piece of content.
Listing Fee: The amount of the SNGLS needed to stake to potentially mine the content onto the platform.
Transaction Fee: The percentage taken on every transaction of a rented or purchased piece of content.
Grants: Capital allocated from the SNGLS DAO Treasury to proposals for development.
Platform Upgrade Proposals: Upgrades to the distribution portal that are voted on by the DAO.
Content Mining: A mechanism that incentivizes multiple parties to come together and decide what content should be allowed on platform.
Staking: The process by which a content creator will put their tokens up for voting for the content to be allowed on the platform or not.
Token Curated Registries: An incentivized voting game that helps create trusted lists that are maintained by the very people that use them.
Peer2Peer (P2P): A computer system where all “peers” are connected to each other via the Internet.  Files can be shared directly between systems on the network without the need for a central server.
